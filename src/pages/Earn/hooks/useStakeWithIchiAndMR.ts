...
import { ICHIVaultDepositGuard, VOLATILITY_THRESHOLD } from "../../../constants/vaults";
...
import { getIchiVaultItemByVaultId, getMRVaultItemByVaultId } from "../utils/getVaultItem";
import { StakeResult } from "./useStake";
import { useVolatility } from "./useVolatility";
import { useVolatilityModal } from "../../../providers/VolatilityModalProvider/useVolatilityModal";
import { MRVaultABI } from "../abi/MRVaultABI";

type UseStakeWithIchiAndMRProps = {
  vaultId: number;
  typedAmount: string;
  setTypedAmount: (val: string) => void;
};

export const useStakeWithIchiAndMR = ({
  vaultId,
  typedAmount,
  setTypedAmount,
}: UseStakeWithIchiAndMRProps): StakeResult => {
  const ichiVaultAddress =
    getIchiVaultItemByVaultId(vaultId)?.vaultAddress ?? zeroAddress;
  const MRVaultAddress = getMRVaultItemByVaultId(vaultId)?.vaultAddress ?? zeroAddress;  

  if (ichiVaultAddress === zeroAddress || MRVaultAddress === zeroAddress) {
    return {
      handleStake: async () => {},
      buttonTitle: 'Stake',
  ...
  const { data: currentVolatility } = useVolatility(ichiVaultAddress);
  const {  showVolatilityWarning } = useVolatilityModal();
  ...
  const { sendTransaction: sendStakeToMRVault, txHash } = useTransaction({
    type: TransactionType.STAKE_OVL,
    successMessage: "Successfully staked!",
  });

  const [stakeStep, setStakeStep] = useState<"idle" | "volatilityCheck" | "wallet" | "pending">("idle");
  ...
  const buttonTitle = useMemo(() => {
    if (stakeStep === "volatilityCheck") return "Check Volatility";
  ...
  const executeDeposit = async () => {
    if (!typedAmount || !account || !publicClient || !walletClient || !depositTokenAddress) {
      setStakeStep("idle");
    ...
    try {
      ...
      const approveMRVaultHash = await sendApproveTransaction({
        address: ichiVaultAddress,
        abi: ichiVaultABI,
        functionName: 'approve',
        args: [MRVaultAddress, stakedTokensAmount],
        account,
        chain,
      });
      if (!approveMRVaultHash) throw new Error('Approve tx failed');

      setStakeStep("pending");

      await sendStakeToMRVault({
        address: MRVaultAddress,
        abi: MRVaultABI,
        functionName: "deposit",
        args: [stakedTokensAmount, MRVaultAddress],
        account,
        chain,
      });
  ...
  const handleStake = async() => {
    setStakeStep("volatilityCheck");
    setAttemptingTransaction(true);
      
    if (currentVolatility !== undefined) {
      const threshold = BigInt(VOLATILITY_THRESHOLD);
  
      if (currentVolatility > threshold) {
        const volatilityPercent = (Number(currentVolatility) / 100).toFixed(2);
        const shouldProceed = await showVolatilityWarning(volatilityPercent);
