...
type UseWithdrawWithIchiAndMRProps = {
  vaultId: number;
  typedAmount: string;
  setTypedAmount: (val: string) => void;
};

export const useWithdrawWithIchiAndMR = ({
  vaultId,
  typedAmount,
  setTypedAmount,
}: UseWithdrawWithIchiAndMRProps) => {
  ...
  const { sendTransaction: sendWithdrawFromMRVault, txReceipt } = useTransaction({
